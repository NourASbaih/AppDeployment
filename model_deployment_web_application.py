...
def main():
    im = Image.open("ml.png")
    st.set_page_config(
    page_title="Requirement Prediction",
    page_icon=im,
    layout="wide",
    )
    ...
    if choice == "Home":
        ...
        st.markdown("- Date source found [here](https://datasetsearch.research.google.com/search?src=0&query=Software%20Requirement&docid=L2cvMTFqY2trMDQ3Ng%3D%3D)")
        st.markdown("- CSV file of the original dataset file can be found [here](https://drive.google.com/file/d/1hJZsMKpM2QRNa-Q3RDpeCOl4E97vZcua/view?usp=sharing)")
        
        st.write("The RandomOverSampler method was used to oversample the minority classes of a given dataset in order to gain a prediction model with high accuracy.")
        st.markdown("- CSV file of the oversampled dataset file can be found [here](https://drive.google.com/file/d/1g3LVHWR8eewbVssPwBQzbC-mmKl-fBa6/view?usp=sharing)")
        
        st.write("The types of requirements in \"Software Rquirement\" dataset are as follows:")
        st.markdown("- Functional (F)")
        st.markdown("- Availability (A)")
        st.markdown("- Fault Tolerance (FT)")
        st.markdown("- Legal (L)")
        st.markdown("- Look & Feel (LF)")
        st.markdown("- Maintainability (MN)")
        st.markdown("- Operational (O)")
        st.markdown("- Performance (PE)")
        st.markdown("- Portability (PO)")
        st.markdown("- Scalability (SC)")
        st.markdown("- Security (SE)")
        st.markdown("- 12-Usability (US)")
